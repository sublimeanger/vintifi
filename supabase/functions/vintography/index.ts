...
const NO_TEXT = `ABSOLUTE RULE #1 — ZERO ADDED TEXT: The output image must contain ABSOLUTELY NO added text, labels, captions, titles, words, letters, numbers, annotations, watermarks, signatures, stamps, banners, "Before"/"After" labels, operation names, mode names, or ANY form of written content whatsoever. NOT EVEN A SINGLE CHARACTER. Do NOT add brand labels, size tags, care labels, or any garment tags that are not clearly visible in the original input image. If an interior label (brand tag, care tag, size tag) is barely visible or partially hidden in the original photo, keep it at the SAME level of visibility — do NOT enhance it, enlarge it, make it more readable, or fabricate text on it. If you cannot read the label text in the original, leave it as an indistinct blur — do NOT guess or generate what it might say. Adding or enhancing any text instantly ruins the image and fails the task.`;
